...
/**
 * 解析 维列斯 协议的头部数据
 * @param {ArrayBuffer} 维列斯Buffer - 维列斯协议的原始头部数据
...
function process维列斯Header(维列斯Buffer, userID) {
    ...
    if (维列斯Buffer.byteLength < 24) {
        return {
            hasError: true,
            message: 'Invalid data: buffer is too short',
        };
    }
    ...
    function isUserIDValid(userID, buffer) {
        ...
        const userIDString = stringify(userIDArray); // 将字节数组转为字符串
        return userIDString === userID || userIDString === userID.toLowerCase();
    }

    // 检查用户 ID 是否有效
    if (!isUserIDValid(userID, 维列斯Buffer)) {
        return {
            hasError: true,
            message: `Invalid user ID: ${(new Uint8Array(维列斯Buffer.slice(1, 17))).toString()}`,
        };
    }
    ...
    if (command === 1) {
        ...
        return {
            hasError: true,
            message: `Unsupported command ${command}, expected 01-TCP, 02-UDP, 03-MUX`,
        };
    }

    // 解析远程端口（大端序，2 字节）
    const portBuffer = 维列斯Buffer.slice(18 + optLength + 1, 18 + optLength + 3);
    portRemote = new DataView(portBuffer).getUint16(0);

    // 解析地址类型和地址
    const addressBuffer = new Uint8Array(维列斯Buffer.slice(18 + optLength + 3, 19 + optLength + 3));
    addressType = addressBuffer[0];

    // 设置地址字段默认值
    let addressLength = 0;
    let addressValue = '';
    let addressValueIndex = 19 + optLength + 3;

    switch (addressType) {
        case 1:
            // IPv4 地址（4 字节）
            addressLength = 4;
            addressValue = new Uint8Array(维列斯Buffer.slice(addressValueIndex, addressValueIndex + addressLength))
                .join('.'); // 转换为点分十进制格式
            break;
                ...
                async write(chunk, controller) {
                    ...
                    try {
                        if (维列斯Header) {
                            ...
                            await webSocket.send(await new Blob([维列斯Header, chunk]).arrayBuffer());
                        ...
                        // 捕获 WebSocket 发送错误
                        console.error(`Error while sending data to WebSocket: ${err.message}`);
                        controller.error(err); // 抛出异常，终止流
                    }
                },
                close() {
                    // 当远程连接的可读流关闭时
...
async function 双重哈希(文本) {
    // 直接编码文本并进行第一次哈希
    const 第一次哈希 = await crypto.subtle.digest('MD5', new TextEncoder().encode(文本));
    ...
    // 使用第一次哈希结果的中间部分进行第二次哈希
    const 第二次哈希 = await crypto.subtle.digest('MD5', new TextEncoder().encode(第一次十六进制.slice(7, 27)));

    // 将第二次哈希结果转为十六进制字符串
    return Array.from(new Uint8Array(第二次哈希))
        .map(byte => byte.toString(16).padStart(2, '0'))
        .join('')
        .toLowerCase();
}
