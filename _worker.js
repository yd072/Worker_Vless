...
/**
 * 解析 维列斯 协议的头部数据
 * @param {ArrayBuffer} protocolBuffer 维列斯协议的原始头部数据
 * @param {string} userID 用于验证的用户 ID
 * @returns {Object} 解析结果，包括是否有错误、错误信息、远程地址信息等
 */
function processViletsHeader(protocolBuffer, userID) {
    // 检查数据长度是否足够（至少需要 24 字节）
    if (protocolBuffer.byteLength < 24) {
        return {
            hasError: true,
            message: 'Invalid data length. Data must be at least 24 bytes.',
        };
    }

    // 解析 维列斯 协议版本（第一个字节）
    const version = new Uint8Array(protocolBuffer.slice(0, 1))[0];
    ...
    function isUserIDValid(userID, buffer) {
        ...
        const userIDString = String.fromCharCode(...userIDArray);
        return userIDString === userID;
    }

    // 使用函数验证
    isValidUser = isUserIDValid(userID, protocolBuffer);

    // 如果用户 ID 无效，返回错误
    if (!isValidUser) {
        return {
            hasError: true,
            message: `Invalid user ID: ${new Uint8Array(protocolBuffer.slice(1, 17))}`,
        };
    }
    ...
    if (command === 1) {
        ...
        return {
            hasError: true,
            message: `Unsupported command ${command}. Valid commands are 01-TCP, 02-UDP, 03-MUX.`,
        };
    }
    ...
    const addressParsers = {
        1: () => {
            ...
            return new Uint8Array(protocolBuffer.slice(addressIndex + 1, addressIndex + 1 + addressLength)).join('.');
        },
        2: () => {
            // 域名
            const addressLength = new Uint8Array(protocolBuffer.slice(addressIndex + 1, addressIndex + 2))[0];
            const domainName = new TextDecoder().decode(protocolBuffer.slice(addressIndex + 2, addressIndex + 2 + addressLength));
            return domainName;
        },
        3: () => {
            // IPv6 地址
            const addressLength = 16;
            const dataView = new DataView(protocolBuffer.slice(addressIndex + 1, addressIndex + 1 + addressLength));
            const ipv6 = [];
            for (let i = 0; i < 8; i++) {
    ...
    // 根据地址类型解析地址
    let addressValue = '';
    if (addressParsers[addressType]) {
        addressValue = addressParsers[addressType]();
    } else {
        return {
            hasError: true,
            message: `Invalid address type ${addressType}. Expected values are 1 (IPv4), 2 (Domain), 3 (IPv6).`,
        };
    }
    ...
    return {
        ...
        addressType,                  // 地址类型
        portRemote,                   // 远程端口
        rawDataIndex: addressIndex + 1 + (addressType === 2 ? new Uint8Array(protocolBuffer.slice(addressIndex + 1, addressIndex + 2))[0] : (addressType === 3 ? 16 : 4)),
        version,                       // 维列斯 协议版本
        isUDP,                         // 是否是 UDP 请求
    };
}
