...
/**
 * 解析 维列斯 协议的头部数据
 * @param { ArrayBuffer} protocolBuffer 维列斯 协议的原始头部数据
...
function processViletsHeader(protocolBuffer, userID) {
    ...
    const addressParsers = {
        1: () => {
            // IPv4 地址
            addressLength = 4;
            addressValue = new Uint8Array(protocolBuffer.slice(addressIndex + 1, addressIndex + 1 + addressLength)).join('.');
        },
        2: () => {
            // 域名
            addressLength = new Uint8Array(protocolBuffer.slice(addressIndex + 1, addressIndex + 2))[0];
            addressValue = new TextDecoder().decode(protocolBuffer.slice(addressIndex + 2, addressIndex + 2 + addressLength));
        },
        3: () => {
            // IPv6 地址
            addressLength = 16;
    ...
    if (!addressValue) {
        return {
            hasError: true,
            message: `Address is empty for addressType ${addressType}.`,
        };
    }
    ...
    return {
        ...
        addressType,                  // 地址类型
        portRemote,                   // 远程端口
        rawDataIndex: addressIndex + 1 + addressLength, // 原始数据的实际起始位置
        version,                       // 维列斯 协议版本
        isUDP,                         // 是否是 UDP 请求
    };
}
