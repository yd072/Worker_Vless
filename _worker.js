...
async function 维列斯OverWSHandler(request) {
    ...
    const writableStream = new WritableStream({
        async write(chunk, controller) {
            if (isDns) {
                return await handleDNSQuery(chunk, webSocket, remoteSocketWapper, log);
            }
            ...
            await handleDnsOrTcp(rawClientData, webSocket, remoteSocketWapper, log);
        },
        close() {
    ...
    await readableWebSocketStream.pipeTo(writableStream).catch((err) => {
