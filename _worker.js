...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            let 动态UUID, 有效时间, 更新时间, proxyIP, socks5Address, enableSocks, parsedSocks5Address;
            ...
            proxyIP = await getRandomElementFromEnv(env.PROXYIP, proxyIP);
            socks5Address = await getRandomElementFromEnv(env.SOCKS5, socks5Address);
...
async function getRandomElementFromEnv(envVar, defaultValue) {
    const elements = await 整理(envVar || defaultValue);
