...
async function 维列斯OverWSHandler(request) {
    try {
        ...
        const log = (info, event = '') => {
            const { address, portWithRandomLog } = state;
            console.log(`[${address}:${portWithRandomLog}] ${info}`, event);
        };

        // WebSocket 关闭事件处理
        webSocket.addEventListener('close', () => {
            if (state.remoteSocketWrapper.value) {
                state.remoteSocketWrapper.value.close();
            }
            log('WebSocket 连接已关闭');
        });
        ...
        await readableWebSocketStream.pipeTo(new WritableStream({
            async write(chunk, controller) {
                ...
                if (state.remoteSocketWrapper.value) {
                    const writer = state.remoteSocketWrapper.value.writable.getWriter();
                    try {
