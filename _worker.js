...
/**
 * 解析维列斯协议的头部数据
 * @param {ArrayBuffer} vesHeaderBuffer 维列斯协议的原始头部数据
 * @param {string} userID 用于验证的用户 ID
 * @returns {Object} 解析结果，包括是否有错误、错误信息、远程地址信息等
 */
function processVesHeader(vesHeaderBuffer, userID) {
  // 检查数据长度是否足够（至少需要 24 字节）
  if (vesHeaderBuffer.byteLength < 24) {
    return {
      hasError: true,
  ...
  // 解析维列斯协议版本（第一个字节）
  const version = new Uint8Array(vesHeaderBuffer.slice(0, 1));
  ...
  function isUserIDValid(userID, buffer) {
    ...
    return userIDString === userID || userIDString === userID.toLowerCase();
  }

  // 使用函数验证
  isValidUser = isUserIDValid(userID, vesHeaderBuffer);

  // 如果用户ID无效，返回错误
  if (!isValidUser) {
    const invalidUserID = new Uint8Array(vesHeaderBuffer.slice(1, 17));
    return {
      hasError: true,
      message: `invalid user ID: ${stringify(invalidUserID)}`,
    };
  }

  // 获取附加选项的长度（第17个字节）
  const optLength = new Uint8Array(vesHeaderBuffer.slice(17, 18))[0];

  // 解析命令（紧跟在选项之后的1个字节）
  // 0x01: TCP, 0x02: UDP, 0x03: MUX（多路复用）
  const command = new Uint8Array(vesHeaderBuffer.slice(18 + optLength, 18 + optLength + 1))[0];
  ...
  if (command === 1) {
    ...
    return {
      hasError: true,
      message: `unsupported command ${command}. Valid commands: 01-tcp, 02-udp, 03-mux`,
    };
  }

  // 解析远程端口（大端序，2字节）
  const portIndex = 18 + optLength + 1;
  const portBuffer = vesHeaderBuffer.slice(portIndex, portIndex + 2);
  const portRemote = new DataView(portBuffer).getUint16(0);

  // 解析地址类型和地址
  let addressIndex = portIndex + 2;
  const addressBuffer = new Uint8Array(vesHeaderBuffer.slice(addressIndex, addressIndex + 1));
  ...
  switch (addressType) {
    case 1:
      ...
      addressLength = 4;
      // 将4个字节转为点分十进制格式
      addressValue = new Uint8Array(vesHeaderBuffer.slice(addressValueIndex, addressValueIndex + addressLength)).join('.');
      break;
    case 2:
      // 域名
      // 第一个字节是域名长度
      addressLength = new Uint8Array(vesHeaderBuffer.slice(addressValueIndex, addressValueIndex + 1))[0];
      addressValueIndex += 1;
      // 解码域名
      addressValue = new TextDecoder().decode(vesHeaderBuffer.slice(addressValueIndex, addressValueIndex + addressLength));
      break;
    case 3:
      // IPv6地址
      addressLength = 16;
      const dataView = new DataView(vesHeaderBuffer.slice(addressValueIndex, addressValueIndex + addressLength));
      const ipv6 = [];
      for (let i = 0; i < 8; i++) {
    ...
    default:
      ...
      return {
        hasError: true,
        message: `invalid address type ${addressType}. Valid types: 1-IPv4, 2-Domain, 3-IPv6`,
      };
  }
