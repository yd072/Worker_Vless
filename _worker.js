...
const base64EncodedProtocol = 'ZG14bGMzTT0=';
const protocolType = atob(base64EncodedProtocol);

function 配置信息(UUID, 域名地址) {
    // 确保 FileName 和 path 已定义
    if (typeof FileName === 'undefined' || typeof path === 'undefined') {
        throw new Error('FileName 或 path 未定义');
    }
    ...
    const 威图瑞 = `${protocolType}://${用户ID}@${地址}:${端口}?encryption=${加密方式}&security=${传输层安全[0]}&sni=${SNI}&fp=${指纹}&type=${传输层协议}&host=${伪装域名}&path=${encodeURIComponent(路径)}#${encodeURIComponent(别名)}`;

    const 猫猫猫 = `- {name: ${FileName}, server: ${地址}, port: ${端口}, type: ${protocolType}, uuid: ${用户ID}, tls: ${传输层安全[1]}, alpn: [h3], udp: false, sni: ${SNI}, tfo: false, skip-cert-verify: true, servername: ${伪装域名}, client-fingerprint: ${指纹}, network: ${传输层协议}, ws-opts: {path: "${路径}", headers: {${伪装域名}}}}`;
