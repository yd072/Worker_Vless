...
class WebSocketManager {
    constructor(webSocket, log) {
        ...
        // 使用批量处理提高效率
        this.BATCH_SIZE = 32; // 批处理大小
        this.PROCESS_INTERVAL = 0; // 处理间隔时间(ms)
    }
    ...
    async processMessageQueue(controller) {
        ...
        while (this.messageQueue.length > 0 && !this.backpressure) {
            try {
                // 批量处理消息
                const batchMessages = this.messageQueue.splice(0, Math.min(this.BATCH_SIZE, this.messageQueue.length));
