...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            let userID = env.UUID || env.uuid || env.PASSWORD || env.pswd;
            if (env.KEY || env.TOKEN || (userID && !isValidUUID(userID))) {
                const 动态UUID = env.KEY || env.TOKEN || userID;
                const 有效时间 = Number(env.TIME) || 0;
                const 更新时间 = Number(env.UPTIME) || 0;
            ...
            const proxyIPs = await 整理(proxyIP);
            proxyIP = proxyIPs[Math.floor(Math.random() * proxyIPs.length)];

            let socks5Address = env.SOCKS5;
            const socks5s = await 整理(socks5Address);
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                if (env.ADD) await 整理(env.ADD);
                if (env.ADDAPI) await 整理(env.ADDAPI);
                if (env.ADDNOTLS) await 整理(env.ADDNOTLS);
                if (env.ADDNOTLSAPI) await 整理(env.ADDNOTLSAPI);
                if (env.ADDCSV) await 整理(env.ADDCSV);
                ...
                if (路径 == '/') {
                    ...
                    const fakeConfig = await 生成配置信息(userID, request.headers.get('Host'), '', 'CF-Workers-SUB', 'false', url, fakeUserID, fakeHostName, env);
