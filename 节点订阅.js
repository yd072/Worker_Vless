...
function parseProxyIP(proxyIP) {
    ...
    let parsedPort = portRemote;

    if (!parsedIP) {
        parsedIP = atob(`UFJPWFlJUC50cDEuZnh4ay5kZWR5bi5pbw==`);
    } else {
        const ipPortPattern = /([^:]+):([^:]+)/;
        const match = parsedIP.match(ipPortPattern);
...
async function retry() {
    try {
        let { parsedIP, parsedPort } = parseProxyIP(proxyIP);

        if (enableSocks) {
            tcpSocket = await connectAndWrite(parsedIP, parsedPort, true);
        } else {
        ...
        try {
            ...
            console.log('Error in remoteSocketToWS:', error);
        }
    } catch (error) {
...
let useSocks = go2Socks5s.length > 0 && enableSocks;
